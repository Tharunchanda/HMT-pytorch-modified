...
parser.add_argument('--task_config', type=str, default='wikitext-2-raw-v1', choices=['wikitext-103-raw-v1', 'wikitext-103-v1', 'wikitext-2-raw-v1', 'wikitext-2-v1','pqa_labeled'],help='Dataset config to use for wikitext')
...
parser.add_argument('--model_name', type=str, default='facebook/opt-1.3b', help='transformer model name for backbone of HMT')
...
parser.add_argument('--test_length', type=int, default=1000, help='context length of input to test')
...
parser.add_argument('--lr_decay', action='store_true', default=True, help='whether having learning rate decay or not')
parser.add_argument('--use_lora', action='store_true', default=True, help='whether use PEFT LoRA to speed up training')
parser.add_argument('--lr_decay_gamma', type=float, default=0.8, help='rate of lr decay')
parser.add_argument('--num_sensory', type=int, default=50, help='number of preserved tokens for sensory memory')
...
parser.add_argument('--train_set_split', type=str, default=None, help='slice upper bound of training set to reduce time for tokenization. use percentage notation (e.g., 2%), or integer')
...
parser.add_argument('--timing', action='store_true', default=True, help='profile the timing of inference.')
...
parser.add_argument('--cache_dir', type=str, default='./small_cache_dir', help='cache directory, default to the current directory')
...
def main():
    ...
    accelerator = Accelerator(kwargs_handlers=[ddp_kwargs], log_with="wandb",mixed_precision='no')
    ...
    if isinstance(model.config, OPTConfig):
        ...
        print(f"word_emb_dim: {word_emb_dim}")
    else:
    ...
    if args.train_set_split is not None:
        split_val = args.train_set_split
        if isinstance(split_val, str) and split_val.endswith('%'):
            frac = float(split_val.strip('%')) / 100.0
            train_count = int(frac * len(train_ds))
        else:
            train_count = int(split_val)
        train_ds = train_ds.shuffle(seed=args.seed).take(train_count)
        # Repeat for valid_ds and test_ds as needed
    if args.train_set_split is not None:
        ...
        test_ds = valid_ds.take(int(args.train_set_split))
        ...
        if args.task_name == 'pubmed_qa':
            ...
            train_ds, valid_ds, test_ds = datasets.load_dataset(args.task_name, args.task_config, split=['train[:75%]', 'train[75%:90%]', 'train[90%:]'])
            ...
            train_ds, valid_ds, test_ds = datasets.load_dataset(args.task_name, args.task_config, split=['train', 'validation', 'test'], trust_remote_code=True)
    ...
    if args.rmt_only or args.baseline_only:
        ...
        if args.hmt_stage_1:
            ...
            if args.load_from_ckpt is not None and args.hmt_stage_2:
                ...
                state_dict_path = os.path.join(args.load_from_ckpt, "model_weights.pth")
    ...
    eval_steps = min(args.eval_step, len(valid_dataloader))
    ...
    if not args.inference_only:
        ...
        for epoch in range(args.num_epochs):
            ...
            for step in tqdm.tqdm(range(min(train_steps, len(train_dataloader)))):
                ...
                tqdm.tqdm.write(f"Processing batch {step + 1}")
                if args.dynamic:
        ...
        if args.save_ckpt is not None:
            ...
            torch.save(model.state_dict(), os.path.join(args.save_ckpt, "model_weights.pth"))
            print(f"Model weights saved to {os.path.join(args.save_ckpt, 'model_weights.pth')}")
    ...
    # Dynamically adjust eval_steps
    eval_steps = min(args.eval_step, len(valid_dataloader))
    logger.info(f"Adjusted eval_steps: {eval_steps}")
    ...
    # Dynamically adjust test_step
    args.test_step = len(test_dataloader)
    logger.info(f"Adjusted test_step: {args.test_step}")

    test_gen = iter(test_dataloader)
    logger.info(f"Test dataset size: {len(test_dataloader)}")
    ...
    try:
        for step in tqdm.tqdm(range(args.test_step)):
            try:
                ...
                logger.warning("Test generator exhausted. Resetting...")
                test_gen = iter(test_dataloader)
                batch = next(test_gen)
            ...
            tqdm.tqdm.write(f'Loss: {loss.item()}, PPL: {ppl.item()}')

            # Log test metrics to wandb
            test_step_offset = train_steps * args.num_epochs
            accelerator.log({"test loss": loss.detach().item(), "test ppl": ppl.detach().item()})
